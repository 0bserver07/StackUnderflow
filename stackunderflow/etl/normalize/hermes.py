...
Hermes writes JSONL sessions where each assistant message carries an explicit usage block:
...
_DEFAULT_MODEL = "hermes-auto"
_RAW_EXTRAS_FIELDS = ("provider", "agentName")
...
def _safe_int(value: object) -> int:
    ...
    if isinstance(value, bytes | bytearray):
        try:
            return max(int(value.decode("utf-8", errors="replace")), 0)
        except ValueError:
            return 0
    return 0
...
def _safe_load_raw(raw_json: object) -> object | None:
    ...
    try:
        if isinstance(raw_json, bytes | bytearray):
            decoded: object = json.loads(raw_json.decode("utf-8", errors="replace"))
            return decoded
