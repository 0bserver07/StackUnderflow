...
//! (`staxtrace.memory/1`, `staxtrace.resume/1`). The envelopes are the
...
//! `staxtrace.memory/1` contract, [`resume`] the `staxtrace.resume/1`
...
pub use envelope::{
    CORE_FIELDS, ErrorEnvelope, MEMORY_SCHEMA, MEMORY_SCHEMA_LEGACY, MEMORY_SCHEMA_VERSION,
    MemoryCommand, MemoryEnvelope, SuccessEnvelope, build_envelope, build_error_envelope,
    is_memory_schema, render, render_line,
};
pub use resume::{
