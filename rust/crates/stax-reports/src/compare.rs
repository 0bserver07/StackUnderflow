...
//! [`crate::mart_queries`], which is still an unported stub owned by another
...
use crate::mart_queries::{mart_has_model_day_rows, mart_has_session_rows, table_exists};
...
///
/// NOT [`crate::mart_queries::iso_to_day`], which is the byte-wise spelling.
...
///
/// DIV-089: [`crate::mart_queries::session_mart_rows_for_compare`] shares the
...
mod tests {
    ...
    fn the_services_error_message_sorts_the_aliases_and_the_routes_does_not() {
        ...
        // …while `routes/compare.py` joins the tuple. Both strings ship, and the
        // OTHER half of this proof now lives where that string does:
        // `stax_server::routes::compare`'s
        // `the_services_error_message_sorts_the_aliases_and_the_routes_does_not`.
        // Splitting it was forced by the tranche-3 crate split (this module moved
        // to `stax-reports`, which must not depend on the HTTP crate); the
        // assertion was RELOCATED, never dropped — a deleted proof is how two
        // strings silently become one.
    }
