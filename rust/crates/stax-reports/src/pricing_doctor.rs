...
//!
//! `cli.py:4896` is `from stackunderflow.routes.pricing import
//! assemble_pricing_health` — the CLI verb imports the *route's* assembler, the
//! same construction `worktrees` uses ("so they never disagree"). `stax-cli`
//! may not link `stax-server` (DIV-279), so the shared home is this crate, and
//! `crates/stax-server/src/routes/pricing.rs` is left as the HTTP shell that
//! parses two query parameters and calls [`assemble_pricing_health`]. That is
//! DIV-375's close applied before the fork rather than after it.
//!
//! # The two things this report gets wrong-looking on purpose
//!
...
//! * **The estimate prices through whatever engine the CALLER built.**
//!   `server.py`'s lifespan flips `infra.costs` onto the `price_book` table
//!   before it serves a byte, so over HTTP the `estimated_delta_usd` figures are
//!   book-priced; `cli.py` never calls `use_price_book_store`, so the CLI verb's
//!   are manifest-priced. That asymmetry is RS-3-082's seam and it is the
//!   reference's, not the port's — hence the engine is a parameter here and the
//!   two callers pass different ones on purpose.
...
///
/// # Errors
/// Any SQLite error from the three sweeps. The freshness probe never errors —
/// an unreadable or unparseable overlay is the `source: "none"` block.
pub fn assemble_pricing_health(
    conn: &Connection,
