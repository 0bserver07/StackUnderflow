//! `routes/quality.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-102` | `GET ` | `/api/static-analysis/session/{session_id}/quality` | `/api/static-analysis/session/{session_id}/quality` |
//! | `RS-5-103` | `POST` | `/api/static-analysis/session/{session_id}/grade  ` | `/api/static-analysis/session/{session_id}/grade` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/quality.py` fills in the handlers and the [`register`] body, and
