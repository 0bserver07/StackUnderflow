//! `routes/patterns.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-088` | `GET ` | `/api/patterns        ` | `/api/patterns` |
//! | `RS-5-089` | `POST` | `/api/patterns/dismiss` | `/api/patterns/dismiss` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/patterns.py` fills in the handlers and the [`register`] body, and
