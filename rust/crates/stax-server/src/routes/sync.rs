//! `routes/sync.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-108` | `GET` | `/api/sync/status  ` | `/api/sync/status` |
//! | `RS-5-109` | `GET` | `/api/sync/overview` | `/api/sync/overview` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/sync.py` fills in the handlers and the [`register`] body, and
