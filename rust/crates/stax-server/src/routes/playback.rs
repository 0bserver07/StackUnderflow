//! `routes/playback.py` — 3 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-091` | `GET` | `/api/playback/{session_id}/fs       ` | `/api/playback/{session_id}/fs` |
//! | `RS-5-092` | `GET` | `/api/playback/{session_id}          ` | `/api/playback/{session_id}` |
//! | `RS-5-093` | `GET` | `/api/playback/project/{project_slug}` | `/api/playback/project/{project_slug}` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/playback.py` fills in the handlers and the [`register`] body, and
