//! `routes/benchmark.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-045` | `GET` | `/api/benchmark          ` | `/api/benchmark` |
//! | `RS-5-046` | `GET` | `/api/benchmark/recommend` | `/api/benchmark/recommend` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/benchmark.py` fills in the handlers and the [`register`] body, and
