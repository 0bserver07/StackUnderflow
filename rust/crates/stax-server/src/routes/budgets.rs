//! `routes/budgets.py` — 3 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-047` | `GET   ` | `/api/budgets` | `/api/budgets` |
//! | `RS-5-048` | `PUT   ` | `/api/budgets` | `/api/budgets` |
//! | `RS-5-049` | `DELETE` | `/api/budgets` | `/api/budgets` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/budgets.py` fills in the handlers and the [`register`] body, and
