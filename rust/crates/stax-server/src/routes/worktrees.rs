//! `routes/worktrees.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-114` | `GET ` | `/api/worktrees          ` | `/api/worktrees` |
//! | `RS-5-115` | `POST` | `/api/worktrees/attribute` | `/api/worktrees/attribute` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/worktrees.py` fills in the handlers and the [`register`] body, and
