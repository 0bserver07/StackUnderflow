//! `routes/meta_agent.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-077` | `GET ` | `/api/meta-agent/tools` | `/api/meta-agent/tools` |
//! | `RS-5-078` | `POST` | `/api/meta-agent/chat ` | `/api/meta-agent/chat` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/meta_agent.py` fills in the handlers and the [`register`] body, and
