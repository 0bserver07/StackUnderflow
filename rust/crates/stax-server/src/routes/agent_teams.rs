//! `routes/agent_teams.py` — 3 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-042` | `GET` | `/api/agent-teams                                      ` | `/api/agent-teams` |
//! | `RS-5-043` | `GET` | `/api/agent-teams/{session_id}                         ` | `/api/agent-teams/{session_id}` |
//! | `RS-5-044` | `GET` | `/api/agent-teams/{session_id}/agent/{agent_session_id}` | `/api/agent-teams/{session_id}/agent/{agent_session_id}` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/agent_teams.py` fills in the handlers and the [`register`] body, and
