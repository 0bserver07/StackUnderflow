//! `routes/live.py` — 2 endpoints, wave 5.
//!
//! | Item | Method | FastAPI path | axum path |
//! |---|---|---|---|
//! | `RS-5-075` | `GET` | `/api/live/stats ` | `/api/live/stats` |
//! | `RS-5-076` | `GET` | `/api/live/stream` | `/api/live/stream` |
//!
//! **Status: not ported.** This file is the slot; the endpoint batch that owns
//! `routes/live.py` fills in the handlers and the [`register`] body, and
