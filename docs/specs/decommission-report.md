...
|---:|---|---|---|
| 1 | DIV-133 | trailing-slash 307 on all 93 endpoints; one `lib.rs` fallback | ⚠ **id collision — see DIV-340**; the request means the app-level row at `TASKS-RS.md:1518`, *not* the `/api/sync/status` row at `:1604` that the sync landing note reports "CLOSED" |
...
| 13 | DIV-200 (hooks) | inject hooks open the store **read-write** from a path whose docstring says read-only | `:1796` — ⚠ **collides with batch E's DIV-200** at `:1978` |
...
DIV-204/batchE (`GET /api/qa/reindex` 404 leg), DIV-234 (`backup verify --name
''`).
...
| R11 | **Two ported endpoints run an LLM / network call on the reference and would become writers if the service is up.** `/api/…/quality` grades via Ollama and `INSERT OR REPLACE`s; if :11434 is ever open, Python runs first and the port would serve **Python's freshly-written grade back** — an accidental near-match hiding a fabricated row. | **PROCEDURAL GUARD ONLY** (DIV-170, DIV-202) — a banner in the case file. A mechanical guard is an `endpoint-parity.sh` change. | The determinism of `M-ollama` is *a property of the machine, not the code*. |
...
| **An Ollama instance actually running** | R11 / DIV-170 / DIV-202 — the quality endpoint becomes a writer and the port could serve Python's fabricated grade back | :11434 is closed on the parity host |
...
2. **Any endpoint whose side effect is another endpoint's input must not share a
   ...
   DIV-200/batchE). One `POST /api/search/reindex` rebuilt a parity home's index
