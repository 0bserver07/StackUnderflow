...
| 4 | Watcher / lock cross-platform coverage. **CI matrix landed** (test + build workflows now run `windows-latest` alongside Ubuntu / macOS at Python 3.11+3.12). The `msvcrt.locking()` lock path and the watchfiles-on-Windows watcher path now execute on every push. Test-marker housekeeping: 10 `test_<provider>_defensive.py` chmod-000 tests + 1 `test_export.py` symlink test skip cleanly on Windows where the underlying OS primitive is a no-op (chmod) or requires elevation (symlink). Local macOS suite still green after the marker additions; Windows verification happens on the first CI push to this branch. Residual risk is documented in CHANGELOG under "Known-gap" — if `msvcrt.locking(LK_NBLCK)` doesn't honour per-(fd, inode) within the same process the way `fcntl.flock` does, the `test_second_acquire_in_same_process_returns_none` case may need a Windows `xfail`. | low (CI now runs the path; gap is observed-or-not on next CI run) |
