...
> **VERSION FREEZE (standing rule, 2026-07-01 — full text in the project `CLAUDE.md`).** Agents never change a version anywhere — `__version__.py`, `pyproject.toml`, `package.json`/lock, `flake.nix`, CHANGELOG `## [N.N.N]` headings, tags, GitHub/PyPI releases, `release:` commits — and never **suggest** a version number. Notes accumulate under `## [Unreleased]` only. History, for the record: the 0.8 → 0.9 jump was agent inflation (`bed5923`, 2026-05-15, twelve hours after 0.8.0), and the v0.9.2 release commit (`59eb59a`, 2026-05-28) was also agent-executed — after the no-bumps directive was already on record. All of it is on PyPI and unrecallable. The version stays at 0.9.x; if the maintainer releases, the increment is the smallest possible (0.9.2 → 0.9.3).
...
1. **VERSION FREEZE — agents never touch versions, period.** `__version__.py`, `pyproject.toml`, `stackunderflow-ui/package.json`/`package-lock.json`, `flake.nix`, CHANGELOG `## [N.N.N]` headings, tags, releases, `release:` commits — and never *suggest* a version number. Accumulate under `## [Unreleased]`. Releases are maintainer-only, smallest-possible increments (0.9.2 → 0.9.3). Full rule: project `CLAUDE.md`.
...
5. **The next release is the maintainer's alone.** `main` carries six eras of unreleased work, all CI-green, accumulated under `## [Unreleased]`. Per the version freeze: do not propose a number, do not prepare a release commit. If the maintainer cuts one, it is the smallest possible increment.
